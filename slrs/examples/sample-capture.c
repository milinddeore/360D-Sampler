...
/************************************ 
 * ARDUINO INTERFACE TO CONTROL MOTOR
 ************************************/
#if defined (ARDUINO_CONNECTED)

void init_arduino_comm(char* portname);
void signal_handler_IO (int status);   /* definition of signal handler */
int read_from_arduino();
void write_to_arduino(char* w_buf);
void close_arduino_fd(void);
...
void init_arduino_comm(char* portname)
{
    ...
    /* Open the file descriptor in blocking mode */
    ard_fd = open(portname, O_RDWR | O_NOCTTY);
    printf("Arduino FD: %d", ard_fd);
    if (ard_fd < 0)
    {
	printf("ERROR: errno: %d cannot open %s for Arduino with error: %s", 
	       errno, portname, strerror(errno));
        exit(1);
    }

    /* Reset Arduino: This is more like shut/no-shut */
    ioctl(ard_fd, USBDEVFS_RESET, 0);
    ...
    /* wait for n ( in our case its 4(float) ) characters to come in before read returns */
    /* WARNING! THIS CAUSES THE read() TO BLOCK UNTIL ALL */
    /* CHARACTERS HAVE COME IN! */
    ...
    /* commit the options */
    if (tcsetattr(ard_fd, TCSANOW, &toptions) != 0)
    {
 ...
 * Read from Arduino
 */
#define CLICKER_ON	0xF1
#define CLICKER_OFF	0xF0

int read_from_arduino()
{
    char clicker_mode = {0};

    /* read up to sizeof(r_buf) bytes from the fd */
    int n = read(ard_fd, &clicker_mode, sizeof(clicker_mode));

    if (clicker_mode != CLICKER_ON)
    {
        printf("Step received in error\n");
    }
    /* print what's in the buffer */
    //cout << n <<" bytes read, buffer contains: " << lc.fp <<endl;
...
void write_to_arduino(char* w_buf)
{
    ...
    if ((bytes_written = write(ard_fd, w_buf, sizeof(w_buf))) < 0)
    {
        printf("ERROR: %d write failed with error", errno);
    }
}

#define SAM_READ_STEP() read_from_arduino()
#define SAM_SEND_START() write_to_arduino("START!")
#define SAM_SEND_STOP() write_to_arduino("STOP!")
#define SAM_SEND_ACK() write_to_arduino("ACK!")
#define SAM_SEND_RESET() write_to_arduino("RESET!")
#define SAM_SEND_REBOOT() write_to_arduino("REBOOT!")


/*
 * Close Arduino fd
 */ 
void close_arduino_fd()
{
    if (close(ard_fd) != 0)
    {
        printf("ERROR: %d fail to close arduino FD with error", errno);
    }
}
...
static void
capture_to_memory(Camera *camera, GPContext *context, const char **ptr, 
...
process:
                ...
                while (rotar_steps < MAX_ROTAR_STEPS)
                {
	                ...
	                capture_to_memory(cams[i], context, (const char**)&data, &size);
                        ...
                        snprintf(fname, sizeof(fname), "/%d-%d.jpg", i, rotar_steps);
